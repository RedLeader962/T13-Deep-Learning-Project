...
def train_rudder(network_lstm, data_test, optimizer, epoches, data_loader, show_gap=5, device='cpu', show_plot=True):
    # Hidden state
    hs = None
    ...
    for epoch in range(epoches):
        ...
        with torch.enable_grad():
            ...
            for data_train in data_loader:
                ...
                # Get samples
                observations, actions, rewards, length = data_train
        ...
        loss_test = validate(network_lstm,data_test, device)

        print(f"Epoch : {epoch}, loss_train: {track_loss / len(data_loader):8.4f}, loss test: {loss_test:8.4f}")

    return network_lstm

def validate(network_lstm, data_test, device):
    hs = None
