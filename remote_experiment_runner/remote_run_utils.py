...
cc = lambda ll: random.choice(ll)


def execute_experiment_plan(exp_specs: List[ExperimentSpec], script_fct: Callable) -> Dict[str, ExperimentSpec]:
    """
    Execute à planned experimentation over a function `script_fct` taking a `ExperimentSpec` dataclass as argument.
    Pre condition:
     1. `ExperimentSpec` variable must be specified
...
def execute_parameter_search(exp_spec: RudderLstmParameterSearchMap,
                             script_fct: Callable,
                             exp_size: int, start_count_at=1,
                             consol_print=True) -> Dict[str, ExperimentSpec]:
